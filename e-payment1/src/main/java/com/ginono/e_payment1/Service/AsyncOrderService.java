...
public class AsyncOrderService {
    ...
    public CompletableFuture<Map<String, Object>> checkOrderStatus(String orderId) {
        try {
            ...
            logger.info("Forwarding order status check to proxy server for order: {}", orderId);

            HttpEntity<Map<String, String>> request = new HttpEntity<>(requestBody, headers);
            
            ResponseEntity<String> rawResponse = restTemplate.exchange(
                PROXY_SERVER_URL + "/check-order-status",
                HttpMethod.POST,
                request,
                String.class
            );
            ...
            logger.info("Order status check forwarded successfully for order: {}", orderId);
            return CompletableFuture.completedFuture(jsonResponse);
        } catch (Exception e) {
